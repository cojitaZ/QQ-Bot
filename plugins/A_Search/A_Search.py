    ...
    async def main(self, event: GroupMessageEvent, debug):
        try:
            ...
            if match:
                ...
                if self.SABI.img_url==None:
                    self.api.groupService.send_group_msg(group_id=event.group_id,message="未选择图片")
                else:
                    ...
                    for engine in self.SABI.engines:
                        ...
                        if len(results)==0:
                            ...
                            for result in results:
                                temp_forward.add_node(type="text",msg=(
                                        f"标题:{result["title"]}\n"+
                    ...
                    await self.api.asyncService.send_group_forward_msg(group_id=event.group_id,forward_message=forward.message)
